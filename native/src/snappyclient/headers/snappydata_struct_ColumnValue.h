...
public:
  ...
  inline typename Visitor::result_type visit(const Visitor& visitor) const {
  ...
  inline void setString(std::string&& val) {
    m_val = std::move(std::make_shared<std::string>(std::move(val)));
  }
  ...
  inline void setDecimal(const Decimal& val) {
    m_val = std::move(std::make_shared<Decimal>(val));
  }
