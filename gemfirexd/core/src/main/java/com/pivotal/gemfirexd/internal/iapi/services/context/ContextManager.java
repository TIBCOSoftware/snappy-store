/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.services.context.ContextManager
...
// GemStone changes BEGIN
		// reduced the default size of stack
...
// GemStone changes BEGIN
	...
	/**
	 * Optimizes access to some context IDs and LCC in particular.
	 * 
	 * Not using concurrent structure here since we do not expect
	 * a connection to be shared by multiple threads and that too
	 * concurrently.
         */	
	static final class ContextMap extends HashMap<Object, Object> {
	  ...
	  public final Object get(Object contextId) {
	    if (contextId == com.pivotal.gemfirexd.internal.iapi.reference
	        .ContextId.LANG_CONNECTION) {
	      return this.lcc;
	    }
	    if (contextId == com.pivotal.gemfirexd.internal.iapi.jdbc
	        .ConnectionContext.CONTEXT_ID) {
	      return this.connCtx;
	    }
	    if (contextId == com.pivotal.gemfirexd.internal.iapi.store.access
	    ...
	    if (contextId == com.pivotal.gemfirexd.internal.iapi.sql.compile
	  ...
	  public final Object put(Object contextId, Object context) {
	    ...
	    else if (contextId == com.pivotal.gemfirexd.internal.iapi.jdbc
                .ConnectionContext.CONTEXT_ID) {
              oldVal = this.connCtx;
              this.connCtx = context;
              return oldVal;
            }
	    else if (contextId == com.pivotal.gemfirexd.internal.iapi.store.access
	        .AccessFactoryGlobals.USER_TRANS_NAME) {
	      oldVal = this.tranCtx;
	      this.tranCtx = context;
	      return oldVal;
	    }
	    else if (contextId == com.pivotal.gemfirexd.internal.iapi.reference
	        .ContextId.LANG_STATEMENT) {
	      oldVal = this.stmtCtx;
	      this.stmtCtx = context;
	      return oldVal;
	    }
	    else if (contextId == com.pivotal.gemfirexd.internal.iapi.sql.compile
	        .CompilerContext.CONTEXT_ID) {
	      oldVal = this.compCtx;
	 ...
	 * @see ContextManager#pushContext(Context)
	 *
	private final Hashtable ctxTable = new Hashtable();
	*/
// GemStone changes END
...
// GemStone changes BEGIN
								  ...
								  idStack.top().getIdName().equals(contextId));
// GemStone changes END
...
// GemStone changes BEGIN
		final int contextIdx = holder.lastIndexOf(theContext);
		if (contextIdx < 0) {
		  return;
		}
		holder.remove(contextIdx);
...
// GemStone changes END
	 ...
	 * @see com.pivotal.gemfirexd.internal.impl.sql.conn.GenericLanguageConnectionContext#resetSavepoints()
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.conn.StatementContext#resetSavePoint()
	 */
...
// GemStone changes BEGIN
		if (error instanceof GemFireException) {
		  StandardException se = Misc.processKnownGemFireException(
		      (GemFireException)error, error,
...
// GemStone changes BEGIN
				  ...
				  // in some rare cases a cleanup might be
				  // going on during shutdown or something
				  // leading to array being cleaned concurrently
				}
// GemStone changes END
				catch (Throwable t) {
			...
			if (reportError) {
// GemStone changes BEGIN
	                        /* (original code) -- another useless logline
...
// GemStone changes BEGIN
				synchronized (this) {
				  if (this.logSeverityLevel == -1) {
				    this.logSeverityLevel = PropertyUtil
...
// GemStone changes BEGIN
	private boolean printStackTrace(Throwable error) {
	  return this.errorStream == null || this.errorStream.printStackTrace(
	      error, this.logSeverityLevel);
	}
// GemStone changes END
...
// GemStone changes BEGIN
		// we set this in the first call in reportError to ensure
		// that GFXD logger is in place
