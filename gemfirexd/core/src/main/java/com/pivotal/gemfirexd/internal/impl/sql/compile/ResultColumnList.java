/*
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ResultColumnList
...
public class ResultColumnList extends QueryTreeNodeVector
{
	...
	*/
	...
	public void treePrint(int depth)
	{
		if (SanityManager.DEBUG)
		{
		        ...
		        if(! SanityManager.DEBUG_ON("PrintRCList") ) {
		          debugPrint("\n");
		          return;
		        }
			...
			throws StandardException
	{
		...
		for (int index = 0; index < size; index++)
		{
			...
			if (rc.isAutoincrementGenerated())
			{
				...
				// Gemstone changes BEGIN
                                userExprFun.push(rc.getTableColumnDescriptor().isGeneratedByDefault());
				// Gemstone changes END

				userExprFun.callMethod(VMOpcode.INVOKEVIRTUAL, ClassName.BaseActivation,
						"getSetAutoincrementValue", ClassName.DataValueDescriptor, /*2*/3 /*GemStoneAddition*/);
				needDVDCast = false;
	...
	*/
					...
					throws StandardException
	{
        ...
        newList.setColumnOrdering(columnOrdering);
		return newList;
	}
	 ...
	 * 
        ...
        public void ncjSetResultSetNumber(int resultSetNumber)
        {
                ...
                for (int index = 0; index < size; index++)
                {
                        ((ResultColumn) elementAt(index)).ncjSetResultSetNumber(resultSetNumber);
                }
        }
...
// GemStone changes BEGIN
	/**
	 * This is a bug in derby as well. Null ordering is not honored when
	 * orderByAndDistinctMerged == true, as it looses this information
	 * in {@link OrderByList#reorderRCL(ResultColumnList)}.
	 * 
	 * This will get cleaned up when other similar issue either we address
	 * or derby fixes it. @see {@link SelectNode#preprocess(int, GroupByList, FromList)}
	 */
	private IndexColumnOrder[] columnOrdering;

	public void setColumnOrdering(IndexColumnOrder[] columnOrdering) {
	...
	public IndexColumnOrder[] getColumnOrdering() {
	  return columnOrdering;
        ...
        public void unmarkAllGenerated() {
        ...
        public final ResultColumn findCommonResultColumn(ResultColumn searchRC) {
          
          ResultColumn    foundRC = null;
          ...
          {
                  ResultColumn    rc =  (ResultColumn) elementAt(index);
                  ResultColumn    childRC = searchRC;
                  
                  if(rc == childRC) {
                    return rc;
                  }
                  
                  while(rc != null || (childRC = childRC.getChildResultColumn()) != null) {
        ...
        public void mergeResultColumnList(ResultColumnList rcl, boolean unmarkGenerated) throws StandardException {
          
          for(int index = 0, size = size(); index < size; index++) {
            
              ResultColumn searchRC = (ResultColumn)elementAt(index);

              if(rcl.findCommonResultColumn(searchRC) == null) {
                
                  ResultColumn newRC = searchRC.cloneMe();
                  if(unmarkGenerated) {
                    searchRC.unmarkGenerated();
                    newRC.unmarkGenerated();
                  }
                  else {
                    newRC.markGenerated();
                  }
      ...
      public void insertResultColumnAt(ResultColumn resultColumn, int index) {
        /* Vectors are 0-based, ResultColumns are 1-based */
        resultColumn.setVirtualColumnId(index + 1);
        insertElementAt(resultColumn, index);
        resetVirtualColumnIds();
      }

      public ResultColumnList eliminateUnUsedColumns(
          ResultColumnList parentResultColumn,
          CollectAndEliminateColumnsVisitor finder) throws StandardException {

        if (parentResultColumn != null) {

          FetchAndSearchColumnReferenceVisitor finder1 = new FetchAndSearchColumnReferenceVisitor();

          ResultColumnList restrictedRCL = (ResultColumnList)getNodeFactory()
              .getNode(C_NodeTypes.RESULT_COLUMN_LIST, getContextManager());

          /*ResultColumnList newDerivedRCL = (ResultColumnList)getNodeFactory()
              .getNode(C_NodeTypes.RESULT_COLUMN_LIST, getContextManager());*/

          
          for (int index = 0; index < this.size(); index++) {
    
            ResultColumn currentRC = ((ResultColumn)this.elementAt(index));
    
            if (currentRC.getExpression() == null || currentRC.isNameGenerated()
                || currentRC.isGenerated()
                || currentRC.isGeneratedForUnmatchedColumnInInsert()
                || currentRC.isExpanded()) {
              restrictedRCL.addElement(currentRC);
              continue;
            }
            
            if(finder.isColumnUsed(currentRC)) {
              currentRC.setVirtualColumnId(restrictedRCL.size()+1);
              restrictedRCL.addElement(currentRC);
              continue;
            }
    
            finder1.setMatchingRC(currentRC);
            
            for (int pIndex = 0; pIndex < parentResultColumn.size(); pIndex++) {
    
              QueryTreeNode parentRC = ((QueryTreeNode)parentResultColumn
                  .elementAt(pIndex));
              parentRC.accept(finder1);
    
              if (finder1.isColumnFound()) {
                currentRC.setVirtualColumnId(restrictedRCL.size()+1);
                restrictedRCL.addElement(currentRC);
                break;
              }
            ...
            /*              finder.setColumnFound(false);
                            for (int cIndex = 0; cIndex < derivedRCL.size(); cIndex++) {
                              ResultColumn currentRC = ((ResultColumn)derivedRCL
                                  .elementAt(cIndex));
                              currentRC.accept(finder);
                              if (finder.isColumnFound()) {
                                newDerivedRCL.addElement(currentRC);
                                break;
                              }
                            }
            */
          ...
          if (restrictedRCL.size() > 0 && restrictedRCL.size() < size()) {
            restrictedRCL.setCountMismatchAllowed(true);
            return restrictedRCL;
            // derivedRCL = newDerivedRCL;
          }
        }
      ...
      /**
       * Merges additional lists of Predicates etc to parentResultColumn if not null otherwise to 
       * the currentResultColumn.
       * 
       * This is to get an aggregated view of the columns being referred.
       * 
       * @param parentResultColumn
       * @param currentResultColumn
       * @param additionalLists
       * @return
       * @throws StandardException
       */
      public ResultColumnList mergeColumnList(ResultColumnList parentResultColumn, QueryTreeNodeVector... additionalLists)
          throws StandardException {
        
        ResultColumnList srcResultColumn = (parentResultColumn != null ? parentResultColumn
            : this);
    
        if(additionalLists == null || additionalLists.length <= 0) {
          return srcResultColumn;
        }
        
        ResultColumnList mergedRCL = (ResultColumnList)getNodeFactory().getNode(
              C_NodeTypes.RESULT_COLUMN_LIST, getContextManager());

        mergedRCL.nondestructiveAppend(srcResultColumn);
        
        for(QueryTreeNodeVector cl : additionalLists) {
          if(cl == null) {
            continue;
          }
          mergedRCL.nondestructiveAppend(cl);
        }
        
        return mergedRCL;
      }

      private static class FetchAndSearchColumnReferenceVisitor extends VisitorAdaptor {

        private ResultColumn matchingRC;
        
        private String currentColumnName;

        private boolean searchMatchingRC = false;
        ...
        public void setMatchingRC(ResultColumn matchingRC) {
          this.matchingRC = matchingRC;
        ...
        public Visitable visit(Visitable node) throws StandardException {
          ...
          if (searchMatchingRC) {
            if (node instanceof ResultColumn) {
              columnFound = ((ResultColumn)node).columnNameMatches(currentColumnName);
            }
            else if (node instanceof ColumnReference) {
              ColumnReference c = ((ColumnReference)node);
              columnFound = c.getColumnName().equals(currentColumnName);
              final ResultColumn src = c.getSource();
              if(src != null) {
            	  columnFound = columnFound || src.columnNameMatches(currentColumnName);
              }
            }
            else if (node instanceof BaseColumnNode) {
              columnFound = currentColumnName.equals(((BaseColumnNode)node).getColumnName());
            }
          }
          else if (node instanceof ResultColumn) {
            currentColumnName =((ResultColumn)node).getName();
            columnFound = matchingRC.columnNameMatches(currentColumnName);
            searchMatchingRC();
          }
          else if (node instanceof ColumnReference) {
            ColumnReference c = ((ColumnReference)node);
            currentColumnName = c.getColumnName();
            searchMatchingRC();
            final ResultColumn src = c.getSource();
            if(src != null) {
               currentColumnName = src.getActualName();
               searchMatchingRC();
            }
          }
          else if (node instanceof BaseColumnNode) {
            currentColumnName = ((BaseColumnNode)node).getColumnName();
            searchMatchingRC();
          }
          else if (node instanceof AggregateNode) {
            // #48488 if its a count(*), all columns are supposed to be expanded, so 
            // don't eliminate any columns.
            columnFound = ((AggregateNode)node).isCountStarAggregate();
          }
          
          return node;
        }
        
        private void searchMatchingRC() throws StandardException {
          searchMatchingRC = true;
          if(!columnFound) {
            this.matchingRC.accept(this);
          }
          searchMatchingRC = false;
        }

        public boolean isColumnFound() {
          return columnFound;
        }
      };
