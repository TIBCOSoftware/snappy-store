...
public class GfxdRemoveGatewayConflictResolverMessage extends AbstractGfxdReplayableMessage {
  ...
  private static final long serialVersionUID = -7742282110492617482L;


  public GfxdRemoveGatewayConflictResolverMessage() {
  }
  ...
  public void execute() throws StandardException {
    ...
    try {
      ...
      if (logger.infoEnabled()) {
        logger.info("GfxdRemoveGatewayConflictResolverMessage: Executing with fields as: "
            + this.toString());
      }
      CallbackProcedures.removeGfxdGatewayConflictResolverLocally();
    } catch (Exception ex) {
      if (logger.fineEnabled()) {
        logger.fine("GfxdRemoveGatewayConflictResolverMessage#execute: exception encountered", ex);
      }
      ...
      if (contextSet) {
        try {
          ...
          if (logger.fineEnabled()) {
            logger.fine("GfxdSetGatewayConflictResolverMessage#execute: exception encountered",
                ex);
          }
        }
  ...
  public byte getGfxdID() {
    return GfxdRemoveGatewayConflictResolverMessage.REMOVE_GATEWAY_CONFLICT_RESOLVER_MSG;
  }
  ...
  public Object getKeyToConflate() {
    return GfxdSetGatewayConflictResolverMessage.CONFLATION_KEY_PREFIX;
  }
