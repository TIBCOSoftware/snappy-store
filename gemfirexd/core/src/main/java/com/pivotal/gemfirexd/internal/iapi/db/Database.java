/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.db.Database
...
/**
 * The com.pivotal.gemfirexd.internal.iapi.db.Database
 * interface provides "internal" methods on the database which are
 * not available to JBMS users (com.pivotal.gemfirexd.internal.database.Database,
...
// GemStone changes BEGIN
	/**
         ...
         * @param isRemote      True if the connection is for a remote node exeuction
         *
         * @return      A new LanguageConnectionContext
         *
         * @exception StandardException thrown if unable to create the connection.
...
// GemStone changes BEGIN
    ...
    /**
     * Any initialization that may need to be done after database has been created
     * and has booted successfully.
     */
    public void postCreate(
        com.pivotal.gemfirexd.internal.iapi.jdbc.EngineConnection conn,
        java.util.Properties bootProps) throws StandardException;

    /**
     * Stores whether to optimize statement to a generic version
     * and avoid recompilations.
     */
    public boolean disableStatementOptimizationToGenericPlan();
    
    public boolean getRuntimeStatistics();
    
    /** Called during boot time failure */ 
