/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.jdbc.EmbedDatabaseMetaData
...
public class EmbedDatabaseMetaData extends ConnectionChild 
	...
	*/
    ...
    /** Cached query descriptions from metadata_lc.properties. */
    private static Properties queryDescriptions_lc;

	/**
     * Return all queries found in either metadata.properties,
     * metadata_net.properties or metadata_lc.properties.
     *
...
// GemStone changes end
	 ...
	 */
// GemStone changes BEGIN
	public ResultSet getTypeInfoForODBC(short dataType)
	    throws SQLException {
	  if (dataType == 0) {
	    return getSimpleQuery("odbc_getTypeInfo");
	  }
	  else {
	    PreparedStatement ps = getPreparedQuery("getTypeInfo2_odbc");
...
// GemStone changes END
	...
	*/
	public boolean supportsGetGeneratedKeys()
	{
                ...
                */
// GemStone changes BEGIN
	  // current support in GemFireXD returns proper columns for generated
	  // columns (limitations in IDENTITY columns themselves as documented)
	  return true;
	  /* (original code)
		return false;
	  */
// GemStone changes END
	...
	*/
	...
	{
// GemStone changes BEGIN
	  // GemFireXD does not support HOLD_CURSORS_OVER_COMMIT (#42803)
...
// GemStone changes BEGIN
		...
		try {
		if (!(spsLocked = spsLock.tryLock(GfxdLockSet.MAX_LOCKWAIT_VAL,
		    TimeUnit.SECONDS))) {
...
// GemStone changes BEGIN
    ...
    // dummy methods so can be compiled by a JDK 1.6+ compiler    
