/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.am.Statement
...
public class Statement implements java.sql.Statement, StatementCallbackInterface{
    ...
    public int sqlMode_ = 0;
    // Enum for sqlMode_:
    public static final int isQuery__ = 0x1; // sql starts with SELECT.... or VALUES...
...
// GemStone changes BEGIN
    protected final static String SET_SCHEMA_STATEMENT =
...
// GemStone changes BEGIN
    protected final java.util.ArrayList specialRegisterArrayList =
...
// GemStone changes END
 ...
 // GemStone changes END
    ...
    private void initStatement() {
        ...
        if (specialRegisterArrayList.size() == 0) {
            specialRegisterArrayList.add(null); // Make sure the list's length is 1
        }
// GemStone changes BEGIN
        initResetStatement(true);
// GemStone changes END
    }

    private void initResetStatement(boolean completeReset) {
        initResetPreparedStatement(completeReset);
    ...
    protected void initResetPreparedStatement(boolean completeReset) {
        warnings_ = null;
        //section_ = null;
        resultSet_ = null;
        updateCount_ = -1;
...
// GemStone changes BEGIN
        if (SanityManager.TraceSingleHop) {
          SanityManager.DEBUG_PRINT(SanityManager.TRACE_SINGLE_HOP,
              "initResetPreparedStatement called with completeReset=" + completeReset/*, new Throwable()*/);
        }
        if (completeReset) {
...
// GemStone changes END
    ...
    private void initStatement(int type, int concurrency, int holdability,
        ...
        // GemStone changes BEGIN
        // throw server side equivalent statement creation time 
        // unsupported exception.
        if (resultSetType_ == java.sql.ResultSet.TYPE_SCROLL_INSENSITIVE
          ...
          // well.
          throw new SqlException(agent_.logWriter_, 
              new ClientMessageId(SQLState.UPDATABLE_SCROLL_INSENSITIVE_NOT_SUPPORTED));
        }
        // GemStone changes END
  ...
  public boolean isClosedNoThrow() {
    return !openOnClient_;
  }
...
// GemStone changes END
    ...
    public void cancel() throws SQLException {
        try
        {
            ...
            // cancel thru thin client works for all prepared statements and 
            // select queries in case of un-prepared statements
            if (this.isPreparedStatement_ || this.isQueryMode()) {
                //  make a new connection
                Properties props = connection_.getConnectionProperties();
                props.setProperty(ClientAttribute.LOAD_BALANCE, "true");
                String host = agent_.getServer();
                String serverPort = Integer.toString(agent_.getPort());
                ...
                "'" + statementUUID + "'" + ")";
                conn.createStatement().execute(cancelStmt);
            } finally {
                if (conn != null) {
                  conn.close();
                }
            ...
            synchronized (connection_) {
                ...
                if (rows < 0 || (maxRows_ != 0 && rows > maxRows_)) {
                    throw new SqlException(agent_.logWriter_, 
                        ...
                        new Integer(rows)).getSQLException(agent_ /* GemStoneAddition */);
                }
                fetchSize_ = rows;
            }
        }
...
// GemStone changes BEGIN                
                ...
                */
                int[] updateCounts = executeBatchX(currExecSeq);
// GemStone changes END
                if (agent_.loggingEnabled()) {
    ...
    private int[] executeBatchX(int currExecSeq) throws SqlException, BatchUpdateException {
        checkForClosedStatement(); // Per jdbc spec (see java.sql.Statement.close() javadoc)
...
// GemStone changes BEGIN        
        ...
        */
        flowExecuteBatch(updateCounts, currExecSeq);
// GemStone changes BEGIN
        return updateCounts;
    }
    ...
    public void writePrepareDescribeOutput(String sql,
        ...
        materialStatement_.writePrepareDescribeOutput_(sql, section, needStatementUUID);
    }
    ...
    public void writeGetStmtUUID(Section section) throws SqlException {
      materialStatement_.writeGetStmtUUID(section);
    }
    
    public void readStatementID() throws SqlException {
      materialStatement_.readStatementID_();
    }
    ...
    public void completeOpenQuery(Sqlca sqlca, ResultSet resultSet) {
        ...
        if (resultSet_.fetchSize_ == 0 &&
            ...
            resultSet_.setFetchSize_(com.pivotal.gemfirexd.internal.client.am.
    ...
    public void completeExecuteCallOpenQuery(Sqlca sqlca, ResultSet resultSet, ColumnMetaData resultSetMetaData, Section generatedSection) {
        ...
        if (resultSet.fetchSize_ == 0) {
            resultSet.fetchSize_ = com.pivotal.gemfirexd.internal.client.am.Configuration.defaultFetchSize;
        }
    }
...
// GemStone changes BEGIN
      ...
      if (SanityManager.TraceClientStatement) {
        ...
        SanityManager.DEBUG_PRINT_COMPACT("flowStmtClose_S", null, cid,
            ns, true, null);
      }
      try {
// GemStone changes END
...
// GemStone changes BEGIN
        ...
        if (SanityManager.TraceClientStatement) {
          final long ns = System.nanoTime();
          SanityManager.DEBUG_PRINT_COMPACT("flowStmtClose_E", null, cid,
              ns, false, null);
        }
      }
// GemStone changes END
...
// GemStone changes BEGIN
      ...
      if (SanityManager.TraceClientStatement) {
        final long ns = System.nanoTime();
        cid = SanityManager.getConnectionId(this.connection_);
        SanityManager.DEBUG_PRINT_COMPACT("flowCloseOUOW_S", null, cid,
            ns, true, null);
      }
      try {
// GemStone changes END
...
// GemStone changes BEGIN
        ...
        if (SanityManager.TraceClientStatement) {
          final long ns = System.nanoTime();
          SanityManager.DEBUG_PRINT_COMPACT("flowCloseOUOW_E", null, cid,
              ns, false, null);
        }
      }
// GemStone changes END
...
// GemStone changes BEGIN
      ...
      if (SanityManager.TraceClientStatement) {
        final long ns = System.nanoTime();
        cid = SanityManager.getConnectionId(this.connection_);
        SanityManager.DEBUG_PRINT_COMPACT("flowCloseRRS_S", null, cid,
            ns, true, null);
      }
      try {
// GemStone changes END
...
// GemStone changes BEGIN
        ...
        if (SanityManager.TraceClientStatement) {
          final long ns = System.nanoTime();
          SanityManager.DEBUG_PRINT_COMPACT("flowCloseRRS_E", null, cid,
              ns, false, null);
        }
      }
// GemStone changes END
...
// GemStone changes BEGIN
      ...
      if (SanityManager.TraceClientStatement) {
        final long ns = System.nanoTime();
        cid = SanityManager.getConnectionId(this.connection_);
        SanityManager.DEBUG_PRINT_COMPACT("flowCloseRRSOUOW_S", null, cid,
            ns, true, null);
      }
      try {
// GemStone changes END
...
// GemStone changes BEGIN
        ...
        if (SanityManager.TraceClientStatement) {
          final long ns = System.nanoTime();
          SanityManager.DEBUG_PRINT_COMPACT("flowCloseRRSOUOW_E", null, cid,
              ns, false, null);
        }
      }
// GemStone changes END
     ...
     * listeners list in <code>com.pivotal.gemfirexd.internal.client.am.Connection</code>.
     * 
     ...
     * 
     * 2. <code>com.pivotal.gemfirexd.internal.client.am.Connection#markStatementsClosed</code> 
     ...
     * <code>com.pivotal.gemfirexd.internal.client.am.Connection</code>.
     */
...
// GemStone changes BEGIN
     ...
     if (SanityManager.TraceClientStatement) {
       final long ns = System.nanoTime();
       cid = SanityManager.getConnectionId(this.connection_);
       SanityManager.DEBUG_PRINT_COMPACT("STMT_flowExecute_S", sql, cid,
           ns, true, null);
     }
     ...
     if (SanityManager.TraceSingleHop | SanityManager.TraceClientHA) {
       SanityManager.DEBUG_PRINT(SanityManager.TRACE_SINGLE_HOP,
           " Statement flowExecute called  sql is "
               + sql + " execution seq: " + currExecSeq);
     }
     try {
...
// GemStone changes BEGIN
                ...
                // This is for implicit savepoint
                newSection.setExecutionSequence(currExecSeq);
// GemStone changes END
                writePrepareDescribeOutput(sql, newSection, needStamentUUID /*GemStone addition*/);
...
// GemStone changes END
            ...
            case isCall__:
// GemStone changes BEGIN
                newSection = writeExecuteCall(sql, false, currExecSeq);
// GemStone changes END
                break;
            }
...
// GemStone changes BEGIN
            if (SanityManager.TraceClientHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                  "set newSection to " + newSection + " for " + this);
            }
// GemStone changes END
...
// GemStone changes BEGIN
       ...
       if (SanityManager.TraceClientStatement) {
         final long ns = System.nanoTime();
         SanityManager.DEBUG_PRINT_COMPACT("STMT_flowExecute_E", sql, cid,
             ns, false, null);
       }
     }
// GemStone changes END
    ...
    void flowExecuteBatch(int[] updateCounts, int currExecSeq) throws SqlException, BatchUpdateException {
// GemStone changes BEGIN
     long cid = 0;
     ...
     if (SanityManager.TraceClientStatement) {
       final long ns = System.nanoTime();
       cid = SanityManager.getConnectionId(this.connection_);
       batchStr = this.batch_.toString();
       SanityManager.DEBUG_PRINT_COMPACT("STMT_flowExecuteBatch_S",
           batchStr, cid, ns, true, null);
     }
     if (SanityManager.TraceSingleHop | SanityManager.TraceClientHA) {
       SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
           " Statement flowExecuteBatch called "
...
// GemStone changes END
        ...
        for (int i = 0; i < batch_.size(); i++) {
          if (doRetry) {
            connection_.incrementAndGetExecutionSequence();
          }
...
// GemStone changes BEGIN
                    section_ = writeExecuteCall(sql, true, currExecSeq);
// GemStone changes END
                }
// GemStone changes BEGIN
                if (SanityManager.TraceClientHA) {
                  SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                      "set section_ to " + this.section_ + " for " + this);
                }
// GemStone changes END
            }
        }
...
// GemStone changes BEGIN
       ...
       if (SanityManager.TraceClientStatement) {
         final long ns = System.nanoTime();
         SanityManager.DEBUG_PRINT_COMPACT("STMT_flowExecuteBatch_E",
             batchStr, cid, ns, false, null);
       }
     }
// GemStone changes END
    }
...
// GemStone changes BEGIN
    private Section writeExecuteCall(String sql,
                                     boolean isBatch, int execSeq) throws SqlException {
// GemStone changes END
        Section newSection = null;
...
// GemStone changes BEGIN
        if (newSection != null) {
          newSection.setExecutionSequence(execSeq);
        }
// GemStone changes END
...
// GemStone changes BEGIN
        if (firstToken.equalsIgnoreCase("<local")
            || firstToken.equalsIgnoreCase("<global")) {
          // read next token
          int idx = sql.indexOf('>');
          if (idx > 0) {
...
// GemStone changes END
        if (firstToken.equalsIgnoreCase("select") || // captures <subselect> production
            // GemStone changes BEGIN
            /*(originally)  firstToken.equalsIgnoreCase("values")) // captures <values-clause> production */
            firstToken.equalsIgnoreCase("values") || // captures <values-clause> production
            firstToken.equalsIgnoreCase("explain")) // captures <explain ...> production
            // GemStone changes END
        {
            sqlMode_ = isQuery__;
    ...
    private String getStatementToken(String sql) throws SqlException {
      return getStatementToken(sql, 0);
    }
     ...
     * @throws SqlException std exception policy
     */
    private String getStatementToken(String sql, int idx) throws SqlException {
    ...
    String cacheCursorAttributesToSendOnPrepare() throws SqlException {
        StringBuilder cursorAttributes = new StringBuilder();
    ...
    void prepareAutoGeneratedKeysStatement() throws SqlException {
        ...
        if (!preparedStatementForAutoGeneratedKeys_.openOnServer_) {
            preparedStatementForAutoGeneratedKeys_.materialPreparedStatement_.writePrepareDescribeOutput_(preparedStatementForAutoGeneratedKeys_.sql_,
                    preparedStatementForAutoGeneratedKeys_.section_, false);
        }
    }
    ...
    public boolean needStatementUUID() {
      return (this.statementUUID == null);
    }
...
// GemStone changes BEGIN
  ...
  public boolean isCallMode() {
    return this.sqlMode_ == isCall__;
  }

  /**
   * prepare added to Statement to allow easy invocation for PreparedStatement
   * from within flow* classes during failover retry from within base Statement
   * methods
   */
  void preparePostFailover() throws SqlException {
    // nothing for Statement
  }
