...
public class IndexPersistenceDUnit extends DistributedSQLTestBase {
  ...
  private int snap1933_restartserver(String tablename, String indexName, int maxValInsert, int netPort, int restartServerNum, Properties serverInfo, int delta) throws Exception {
    Thread.sleep(3000);
    ...
    for (int i = 0; i < 10; i++) {
      int id = delta1 * i + 31 + delta;
      int r1 = s1.executeUpdate("update ODS."
          + tablename
          + " set cd_typ_ref_id=" + id + 10 + " where CD_VAL_ID=" +  id);
      assert(r1 == 1);
    }
    restartServerVMNums(new int[] { 1 }, 0, null, serverInfo);
    Thread.sleep(3000);

    // Start network server on the VMs
    final int netPort2 = startNetworkServer(restartServerNum, null, null);
    Connection conn2 = TestUtil.getNetConnection(netPort2, null, null);
    Statement s2 = conn2.createStatement();
    ResultSet rs2 = s2.executeQuery("select count(*) from ODS."
        + tablename
    ...
    while(rs2.next()) {
      ...
      assertTrue("Got value=" + l, l == maxValInsert);
    }
    return netPort2;
  }

  public void verifySNAP1933(int redundancy, String tablename, String indexname) throws Exception {
    ...
    serverSQLExecute(1, "Create table "
        ...
        + " BUCKETS 1"
        + " EVICTION BY LRUCOUNT 1 EVICTACTION OVERFLOW PERSISTENT ASYNCHRONOUS");
    ...
    for (int i = 1; i < maxValInsert + 1; i++) {
      ...
      serverSQLExecute(1, "insert into ODS." + tablename
          + " values(" +  intVal + "," + 1997 + intVal + "," + intVal + ",'nCZTBAdWBJslcVrJhumxuE'," + intVal + 328647 + ",'fsHxXsyktJ','tENSZGopJOKFTNzKLnWPlAMnjNPJKXBfWhbFFK','2012-01-18 15:13:43','2012-01-18 13:08:07')");
    }

    sqlExecuteVerify(new int[] {}, new int[] { 1, 2 }, "select count(*) from ODS."
        + tablename, null, "" + maxValInsert);

    netPort1 = snap1933_restartserver(tablename, indexname, maxValInsert, netPort2, 1, serverInfo, 0);
    netPort2 = snap1933_restartserver(tablename, indexname, maxValInsert, netPort3, 2, serverInfo, 1);
    netPort3 = snap1933_restartserver(tablename, indexname, maxValInsert, netPort1, 3, serverInfo, 2);

    netPort1 = snap1933_restartserver(tablename, indexname, maxValInsert, netPort2, 1, serverInfo, 4);
    netPort2 = snap1933_restartserver(tablename, indexname, maxValInsert, netPort3, 2, serverInfo, 5);
    netPort3 = snap1933_restartserver(tablename, indexname, maxValInsert, netPort1, 3, serverInfo, 6);
    ...
    sqlExecuteVerify(new int[] {}, new int[] { 1, 2, 3 }, "select count(*) from ODS."
        ...
        + indexname
        + "", null, "" + maxValInsert);
    serverSQLExecute(1, "Drop index " + " ODS." + indexname);
    serverSQLExecute(1, "Drop table " + " ODS." + tablename);
  }

  public void testSNAP1933() throws Exception {
    // TODO - will be investigated for another issue verifySNAP1933(1, "CODE_VALUE_1", "CODE_VAL_REF_ID_1");
    verifySNAP1933(2, "CODE_VALUE_2", "CODE_VAL_REF_ID_2");
    verifySNAP1933(3, "CODE_VALUE_3", "CODE_VAL_REF_ID_3");
  }
}
