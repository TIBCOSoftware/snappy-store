...
public final class RegionAdvisor extends CacheDistributionAdvisor {
    ...
    synchronized(this.preInitQueueMonitor) {
      ...
      try {
        while(pi.hasNext()) {
          ...
          if (!qbp.isRemoval) {
            if (VERBOSE) {
              logger.info(
                  ...
                  "applying queued profile addition for bucket " 
                    + qbp.bucketId);
            }
          ...
          if (qbp.memberDeparted || 
            ...
            if (VERBOSE) {
              logger.info(
                  LocalizedStrings.ONE_ARG,  
                  "applying queued member departure for all buckets for " 
                  + qbp.memberId);
            }
            ...
            // in some rare cases the length of serials array can be zero
            // when remote region creation failed during initialization (#47074)
            final int len = Math.min(this.buckets.length, qbp.serials.length);
            for (int i = 0; i < len; i++) {
              BucketAdvisor ba = this.buckets[i].getBucketAdvisor();
        ...
        if (!finishedInitQueue && 
          ...
          getDistributionManager().getLoggerI18n().error(
    ...
    if (getPartitionedRegion().isDataStore()) {
      for (Integer bucketId : getPartitionedRegion().getDataStore().getAllLocalBucketIds()) {
        ...
        if (getLogWriter().fineEnabled()) {
          getLogWriter().fine("The local profile is : " + profile);
        }
  ...
  public ConcurrentHashMap<Integer, Set<ServerBucketProfile>> getAllClientBucketProfilesTest() {
    ...
    if (getLogWriter().fineEnabled()) {
      getLogWriter().fine("This maps is sksk " + map + " and size is " + map.keySet().size());
    }
    return map;
  }
...
//      LogWriterI18n log = InternalDistributedSystem.getLoggerI18n();
    ...
    if (VERBOSE) {
      getLogWriter().info(LocalizedStrings.DEBUG,
          "RegionAdvisor#removeId: removing member from region "
          + this.getPartitionedRegion().getName() + ": " + memberId
          + "; removed = " + removedId + "; crashed = " + crashed
//          , new Exception("stack trace")
      ...
      int prSerial, int serials[], boolean regionDestroyed)
  {
    if (VERBOSE) {
      getLogWriter().info(
          LocalizedStrings.RegionAdvisor_REGIONADVISOR_REMOVEIDANDBUCKETS_REMOVING_MEMBER_FROM_REGION_0_1,
          new Object[] {this.getPartitionedRegion().getName(), memberId
          + "; buckets = " 
//          + Arrays.toString(serials)
          + (serials == null ? "null" : "(" + serials.length + " serials)")
          }
//          , new Exception("stack trace")
          );
    }
    ...
    if (this.buckets != null) {
      if (VERBOSE) {
        getLogWriter().info(
            LocalizedStrings.RegionAdvisor_REGIONADVISOR_REMOVEIDANDBUCKETS_REMOVING_BUCKETS_FOR_MEMBER0_1,
      ...
      for (int i = 0; i < this.buckets.length; i++) {
        ...
        if (s != ILLEGAL_SERIAL) {
          if (VERBOSE) {
            getLogWriter().info(
                LocalizedStrings.ONE_ARG,
                "RegionAdvisor#removeIdAndBuckets: removing bucket #" + i 
                + " serial " + s);
          }
...
//      super.removeId(memberId);
  ...
  public void markBucketsOnMember(DistributedMember member, boolean sick){
    ...
    for(int i=0; i<buckets.length; i++){
      ...
      if(getLogWriter().fineEnabled()){
        getLogWriter().fine("Marked bucket ("+getPartitionedRegion()
            .bucketStringForLogs(i)+") "+(this.buckets[i].isBucketSick()
                ? "sick":"healthy"));
      }
    }
  }
  ...
  public void checkIfBucketSick(final int bucketId, final Object key) throws LowMemoryException{
    ...
    if (this.buckets[bucketId].isBucketSick()) {
      ...
      if (getLogWriter().fineEnabled()) {
        getLogWriter().fine("For bucket "+getPartitionedRegion().bucketStringForLogs(bucketId)+
            " sick members are "+ sm);
      }
...
//    }
  ...
  public Set<InternalDistributedMember> adviseDataStore(boolean realHashSet) {
    ...
    if (VERBOSE) {
      getLogWriter().info(
          LocalizedStrings.ONE_ARG,  
          "adviseDataStore returning " + s + " from " + toStringWithProfiles());
    }
    return s; 
  }
  ...
  public Set<InternalDistributedMember> adviseFixedPartitionDataStores(
    ...
    if (VERBOSE) {
      getLogWriter().info(
          LocalizedStrings.ONE_ARG,
          "adviseFixedPartitionDataStore returning " + s + " from "
  ...
  public InternalDistributedMember adviseFixedPrimaryPartitionDataStore(
    ...
    if (VERBOSE) {
      getLogWriter().info(
          LocalizedStrings.ONE_ARG,
          "adviseFixedPartitionDataStore returning " + fixedPartitionDataStore
              + " from " + toStringWithProfiles());
    }
...
//    final LogWriterI18n log = ((LocalRegion)event.getRegion()).getCache().getLoggerI18n();
//    final boolean isPutAll = event.getOperation().isPutAll();
//    if (isPutAll && log.fineEnabled()) {
//      log.fine("adviseRequiresNotification found putAll event");
//    }
    return adviseFilter(new Filter() {
      public boolean include(Profile profile) {
        if (profile instanceof PartitionProfile) {
  ...
  public Map<Integer,BucketAdvisor> getAllBucketAdvisorsHostedAndProxies() {
    Assert.assertTrue(this.buckets != null);
    Map<Integer,BucketAdvisor> map = new HashMap<Integer,BucketAdvisor>();
    for(int i=0; i<buckets.length; i++){
      ProxyBucketRegion pbr = this.buckets[i];
      ...
      if (ret != null) {
        ...
        BucketAdvisor bad = pbr.getBucketAdvisor();
        map.put(pbr.getBucketId(), bad);
      }
    }
    return map;
  }
  ...
  // For GemFireXD ALTER TABLE, need to reset the parentAdvisors if colocated
  // region changes
  ...
  /**
   * Return an initialized node favored for reading for the given bucket
   * 
   * @param bucketId
   *          the bucket we want to read
   * @param skipUninitializedMembers
   *          if true then skip the members that are
   *          {@link GemFireCacheImpl#isUnInitializedMember}
   * @return the member, possibly null if no member is available
   */
  public final InternalDistributedMember getPreferredInitializedNode(
      final int bucketId, final boolean skipUninitializedMembers) {
    Assert.assertTrue(this.buckets != null);
    final Bucket b = this.buckets[bucketId];
    return b.getBucketAdvisor().getPreferredInitializedNode(
        skipUninitializedMembers);
  }
  ...
  public final ProxyBucketRegion[] getProxyBucketArray() {
    return this.buckets;
  }
  ...
  /** 
   * Dumps out all profiles in this advisor AND all buckets.
   * @param log the LogWriterI18n to write profile dump to
   * @param infoMsg prefix message to log
   */
  @Override
  public void dumpProfiles(LogWriterI18n log, String infoMsg) {
    log.info(LocalizedStrings.DEBUG, "[dumpProfiles] dumping " + this.toStringWithProfiles());
    
    // 1st dump all profiles for this RegionAdvisor
    super.dumpProfiles(log, infoMsg);
  ...
  //TODO: Asif: The method below may return
  //incomplete number of total primaries as some of the nodes hosting primary buckets  may be down.
  //In that sense, it will give only the current primaries of the system
  /**
  ...
  public Set adviseBucketProfileExchange() {
    // for TX, also need to send to accessors so that they can send
    // TX and commit ops directly to new node
    return adviseAllPRNodes();
