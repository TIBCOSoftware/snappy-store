...
public final class JGroupMembershipManager implements MembershipManager {
  ...
  /**
   * Should we enable JavaGroup tracing?
   */
  public static boolean DEBUG_JAVAGROUPS = Boolean.getBoolean("JGroups.DEBUG");
  
  /**
   * Verbose monitoring of JGroups views.  This or fine logging
   * enables logging (at the info level) of this module.
   */
...
//  public final static boolean multicastTest = false;
  ...
  /**
   * This is the logger, for tracking purposes
   */
   ...
   */
  protected volatile NetView latestView = new NetView();
...
//  private boolean enableClockStats = DistributionStats.enableClockStats;
  ...
  private final boolean enableJgStackStats = Boolean.getBoolean("p2p.enableJgStackStats");
  ...
  private final Object sendSuspendMutex = new Object();
   ...
   MyDCReceiver(DistributedMembershipListener up) {
      ...
      if (verifySuspectProtocol != null) {
        if (addr.getBirthViewId() < 0) {
          if (logger.fineEnabled()) {
            ...
            logger.fine("Membership: invalid address found in sender of " + msg);
          }
        } else {
    ...
    public void memberDeparted(InternalDistributedMember id, boolean crashed, String reason)
    {
      try {
        ...
        logger.warning(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_ERROR_HANDLING_MEMBER_DEPARTURE__0, e);
      }
    }
...
//        logger.severe("Illegal member detected from JavaGroups view: " + dm);
//        members.remove(i);
  ...
  protected void processView(long newViewId, NetView newView)
  {
    // Sanity check...
     if (VERBOSE_VIEWS || DistributionManager.VERBOSE) {
      ...
      logger.info(LocalizedStrings.DEBUG, msg);
...
//     }
    ...
    try {
      ...
      for (Iterator<Map.Entry<InternalDistributedMember, Long>> it=surpriseMembers.entrySet().iterator(); it.hasNext(); ) {
        ...
        short itsVersion = mbr.getVersionOrdinal();
        if (0 < itsVersion && itsVersion < version) {
          version = itsVersion;
        }
      }
      for (InternalDistributedMember mbr: newView) {
        short itsVersion = mbr.getVersionOrdinal();
        if (0 < itsVersion && itsVersion < version) {
          version = mbr.getVersionOrdinal();
        }
      }
      disableMulticastForRollingUpgrade = version != Version.CURRENT_ORDINAL;
      
      if (newViewId < lastViewId) {
        // ignore this view since it is old news
        if (newViewId < lastViewId && (VERBOSE_VIEWS || logger.fineEnabled())) {
          logger.info(
              LocalizedStrings.DEBUG,
              "Membership: Ignoring view (with id " + newViewId + ") since it is older than the last view (with id "
              + lastViewId + "); ignoredView=" + DistributionManager.printView(newView));
        }
        return;
      }
...
//        if (isShunned(m)) {
//          warnShuns.add(m);
////          logger.info("Membership: already shunned " + m);
//          continue;
//        }
...
//        logger.info("Membership: already seen " + m);
        ...
        if (shutdownInProgress()) {
          if (VERBOSE_VIEWS || DistributionManager.VERBOSE) {
            logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_SHUNNING_MEMBER__0__DURING_OUR_SHUTDOWN, m);
          }
          addShunnedMember(m);
          ...
          if (wasShunned  &&  (VERBOSE_VIEWS || DistributionManager.VERBOSE)) {
            logger.info(LocalizedStrings.DEBUG, "No longer shunning " + m + " as it is in the current membership view");
          }
        }
        
        if (VERBOSE_VIEWS || DistributionManager.VERBOSE) {
          logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_PROCESSING_ADDITION__0_, m);
        }
        ...
        try {
          ...
          logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_FAULT_WHILE_PROCESSING_VIEW_ADDITION_OF__0, m, t);
        }
      } // additions
      ...
      for (int i = 0; i < priorView.size(); i++) { // departures
        ...
        try {
          if (VERBOSE_VIEWS || DistributionManager.VERBOSE) {
            logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_PROCESSING_DEPARTING_MEMBER__0_, m);
          }
          ...
          // is still usable:
          SystemFailure.checkFailure();
          logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_FAULT_WHILE_PROCESSING_VIEW_REMOVAL_OF__0, m, t);
        }
      } // departures
      ...
      for (Iterator<Map.Entry<InternalDistributedMember, Long>> it=surpriseMembers.entrySet().iterator(); it.hasNext(); ) {
        ...
        if (birthtime.longValue() < oldestAllowed) {
          ...
          logger.info(
            LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_EXPIRING_MEMBERSHIP_OF_SURPRISE_MEMBER_0,
            m);
      ...
      for (Iterator it=suspectedMembers.entrySet().iterator(); it.hasNext(); ) {
        ...
        if (birthtime.longValue() < oldestAllowed) {
          ...
          it.remove();
          if (logger.fineEnabled() || VERBOSE_VIEWS) {
            logger.info(
                LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_EXPIRING_SUSPECT_MEMBER_0, m);
          }
        }
      }
      try {
    ...
    public void receive(Message jgmsg) {
      ...
      if ( (vmKind == DistributionManager.ADMIN_ONLY_DM_TYPE)
        ...
        if (logger.fineEnabled() && DistributionManager.VERBOSE)
          logger.fine("Membership: admin VM discarding cache operation message " + jgmsg.getObject());
        return;
      }
      ...
      try {
        ...
        if (e.getCause() != null && e.getCause() instanceof java.io.IOException) {
          logger.error(
              LocalizedStrings.JGroupMembershipManager_EXCEPTION_DESERIALIZING_MESSAGE_PAYLOAD_0, 
              jgmsg, e.getCause());
          return;
        }
        else {
      ...
      if (o == null) {
        logger.warning(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_GEMFIRE_RECEIVED_NULL_MESSAGE_FROM__0, String.valueOf(jgmsg));
        logger.warning(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_MESSAGE_HEADERS__0, jgmsg.printObjectHeaders());
        return;
      }
      ...
      if (sender == null) {
        ...
        logger.warning(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_GEMFIRE_RECEIVED_A_MESSAGE_WITH_NO_SENDER_ADDRESS, e);
      }
    ...
    public void channelClosing(Channel c, final Exception e) {
      ...
      if (JGroupMembershipManager.this.puller != null // not shutting down
        ...
        if (logger instanceof ManagerLogWriter) {
          ...
          ((ManagerLogWriter)logger).shuttingDown();
        }
        ...
        if (!inhibitForceDisconnectLogging) {
          logger.severe(
              LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_SERVICE_FAILURE_0,
              reason, logException);
        }
...
//            }
   ...
   *          the {@link LogWriterI18n} to use for security related messages
   * @param transport
   ...
   */
  private JChannel createChannel(LogWriterI18n theLogger, 
      LogWriterI18n securityLogger, RemoteTransportConfig transport, 
      DistributionConfig theConfig)
  {
    Assert.assertTrue(theLogger != null, "Null logger???");
...
//    }
    ...
    // Use this LogWriterI18n when logging Jgroups output
    if (DEBUG_JAVAGROUPS) {
      GemFireTracer.getLog(JGroupMembershipManager.class).setLogWriter(
          theLogger, true);
    } else {
      GemFireTracer.getLog(JGroupMembershipManager.class).setLogWriter(
          theLogger, GemFireTracer.DEBUG);
    }
    GemFireTracer.getLog(JGroupMembershipManager.class).setSecurityLogWriter(
        securityLogger);
    ...
    if (JAVAGROUPS_CONFIG != null) {
      ...
      try {
        if (theLogger.fineEnabled()) {
          theLogger.fine("Reading Jgroups config from " + file);
        }
    ...
    long fdTimeout = Long.getLong("gemfire.FD_TIMEOUT", 0).longValue(); // this is what we told Bear to set in 4.1.2 to force use of FD
...
//    AuthHeader.MAX_CREDENTIAL_SIZE = theConfig.getUdpFragmentSize() - 1000;
    ...
    // so that a system like Amdocs will have a shorter timeout.  We don't want a longer
    // timeout than 12437 because new members will likely time out trying to connect
    // because their join timeouts are set to expect a shorter period
    ...
    if (theLogger.fineEnabled()) {
      theLogger.fine("Jgroups configuration: " + properties);
    }
    ...
    try {
      try {
        ...
        if (DEBUG_JAVAGROUPS) {
          this.logger.fine("Connected JGroups stack: " + myChannel.printProtocolSpec(false));
        }
        ...
        if (VERBOSE_VIEWS || theLogger.fineEnabled())
          theLogger.info(
              LocalizedStrings.DEBUG,
        ...
        if (DistributionManager.VERBOSE) {
          if (theLogger.infoEnabled()) {
            theLogger
            .info(LocalizedStrings.JGroupMembershipManager_CONNECTED_TO_JGROUPS_CHANNEL_TOOK__0__MS, delta);
          }
        }
        ...
        if (ex.getCause() != null && ex.getCause() instanceof GemFireConfigException) {
          throw (GemFireConfigException)(ex.getCause());
        }
...
//logger.severe("getDisableTcp() returned false for " + config.getClass().getName(), new Exception("stack trace"));
//logger.severe("DistributionConfigImpl.forceDisableTcp="+DistributionConfigImpl.forceDisableTcp);
      directChannel = new DirectChannel(this, dcReceiver, config, this.logger, null);
    }
    ...
    try {
      this.channel = createChannel(logger, securityLogger, transport, config);
    }
    catch (RuntimeException e) {
  ...
  public void quorumLost(Set failures, List remaining) {
    ...
    if (notify) {
      if (inhibitForceDisconnectLogging) {
        this.logger.info(LocalizedStrings.DEBUG, "<ExpectedException action=add>Possible loss of quorum</ExpectedException>");
      }
      this.logger.severe(LocalizedStrings.JGroupMembershipManager_POSSIBLE_LOSS_OF_QUORUM_DETECTED, new Object[] {failures.size(), failures});
      if (inhibitForceDisconnectLogging) {
        this.logger.info(LocalizedStrings.DEBUG, "<ExpectedException action=remove>Possible loss of quorum</ExpectedException>");
      }
  ...
  protected void removeWithViewLock(InternalDistributedMember dm,
    ...
    if (VERBOSE_VIEWS || logger.fineEnabled()) {
      logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_DISPATCHING_UPLEVEL_DEPARTURE_EVENT_FOR__0_, dm);
    }
  ...
  public boolean addSurpriseMember(DistributedMember dm, 
    ...
    try {
      ...
      if (latestView.getViewNumber() > member.getVmViewId()) {
        ...
        new Thread(Thread.currentThread().getThreadGroup(),
          ...
          public void run() {
            ...
            logger.severe(LocalizedStrings.JGroupMembershipManager_Invalid_Surprise_Member, new Object[]{member, latestView});
      ...
      if (shutdownInProgress()) {
        if (VERBOSE_VIEWS || logger.fineEnabled()) {
          logger.info(
            LocalizedStrings.DEBUG,
            "Membership: new member during shutdown ignored: <" 
            + member.toString() + ">");
        }
      ...
      if (isShunned(member)) {
        ...
        if (VERBOSE_VIEWS || DistributionManager.VERBOSE) {
          logger.info(LocalizedStrings.DEBUG, "Membership: Processing surprise addition <" + member + ">");
        }

        // make sure the surprise-member cleanup task is running
        // [sumedh] listener cannot be an instanceof DistributionManager
        /*
    ...
    if (warn) { // fix for bug #41538 - deadlock while alerting
      logger.warning( 
          LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_IGNORING_SURPRISE_CONNECT_FROM_SHUNNED_MEMBER_0,
          member);
    } else {
  ...
  private void startCleanupTimer() {
    ...
    try {
      ...
      if (ds != null && ds.isConnected()) {
        this.cleanupTimer = new SystemTimer(ds, true, logger);
  ...
  protected void handleOrDeferViewEvent(View viewArg) {
    ...
    try {
      ...
      if (VERBOSE_VIEWS || DistributionManager.VERBOSE) {
        logger.info(
            LocalizedStrings.DEBUG,
            "Membership: queuing new view for processing, id = " 
            + newId + ", view = " + newView);
      }
  ...
  public void startEventProcessing()
  {
    ...
    synchronized (startupMutex) {
      if (logger.fineEnabled())
        logger.fine("Membership: draining startup events.");
      ...
      for (;;) {
        ...
        synchronized (startupLock) {
          ...
          if (logger.fineEnabled())
            logger.fine("Membership: " + remaining + " remaining startup message(s)");
        ...
        try {
          ...
          // is still usable:
          SystemFailure.checkFailure();
          logger.warning(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_ERROR_HANDLING_STARTUP_EVENT, t);
        }
        
      } // for
      if (logger.fineEnabled())
        logger.fine("Membership: finished processing startup events.");
    } // startupMutex
  }
  ...
  public void waitForEventProcessing() throws InterruptedException {
    ...
    if (processingEvents)
      return;
    logger.fine("Membership: waiting until the system is ready for events");
    for (;;) {
    ...
    logger.fine("Membership: continuing");
  }
  ...
  public NetView getView()
  {
    ...
    for (int i = 0; i < v.size(); i ++) {
      ...
      if (isShunned(m)) {
//        if (VERBOSE_VIEWS || logger.fineEnabled()) {
//            logger.fine("getView(): excluding shunned member: " + m);
//        }
        continue;
      }
      result.add(m);
  ...
  /**
   * @see MembershipManager#getViewId()
   */
  public long getViewId() {
    // take a snap of latest view
    final NetView v = this.latestView;
    return v.getViewNumber();
    /*
    if (v != null) {
      return v.getViewNumber();
    }
    else {
      // also check for cache being closed
      GemFireCacheImpl.getExisting().getCancelCriterion()
          .checkCancelInProgress(null);
      Exception e = this.shutdownCause;
      if (e != null) {
        throw new DistributedSystemDisconnectedException(
            "Membership services are closed", e);
      }
      // the conduit may have been closed
      if (this.conduit != null) {
        this.conduit.getCancelCriterion().checkCancelInProgress(null);
      }
      // cannot find view and not shutting down
      throw new InternalGemFireError("JGMM.getViewId: unexpected null View");
    }
    */
  }

  /**
   * @see MembershipManager#forEachViewMember(TObjectProcedure, boolean)
   */
  public void forEachViewMember(final TObjectProcedure proc,
    ...
    try {
      ...
      for (int index = 0; index < v.length; index++) {
        ...
        if (!excludeShunned || !isShunnedNoSync(m)) {
          if (!proc.execute(m)) {
            break;
          }
        }
  ...
  {
    if (channelPause > 0) {
      logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_PAUSING_TO_ALLOW_OTHER_CONCURRENT_PROCESSES_TO_JOIN_THE_DISTRIBUTED_SYSTEM);
      try {
...
//    memberToStubMap.clear();
  ...
  public void shutdownMessageReceived(InternalDistributedMember id, String reason) {
    if (logger.fineEnabled()) {
      logger.fine("Membership: recording shutdown status of " + id);
    }
  ...
  public void shutdown()
  {
    ...
    if (logger.fineEnabled()) {
      logger.fine("Membership: channel closed");
    }
  }
...
//    this.logger.warning(LocalizedStrings.DEBUG, "DEBUG: JGMM.uncleanShutdown invoked");
    ...
    if (e != null) {
      try {
        ...
        logger.warning(LocalizedStrings.JGroupMembershipManager_EXCEPTION_CAUGHT_WHILE_SHUTTING_DOWN, re);
      }
    }
  }
    ...
    if (!dconfig.getDisableAutoReconnect()) {
      ...
      if (cache != null && (cache instanceof Cache)) {
        if (!Boolean.getBoolean("gemfire.autoReconnect-useCacheXMLFile")
          ...
          try {
            if (this.logger.fineEnabled()) {
              this.logger.fine("generating XML to rebuild the cache after reconnect completes");
            }
            StringPrintWriter pw = new StringPrintWriter(); 
            CacheXmlGenerator.generate((Cache)cache, pw, false, false);
            String cacheXML = pw.toString();
            cache.getCacheConfig().setCacheXMLDescription(cacheXML);
            if (this.logger.fineEnabled()) {
              this.logger.fine("XML generation completed: " + cacheXML);
            }
          } catch (CancelException e) {
            this.logger.info(LocalizedStrings.JGroupMembershipManager_PROBLEM_GENERATING_CACHE_XML, e);
          }
        }
      }
    }
  ...
  public boolean requestMemberRemoval(DistributedMember mbr, String reason) {
    ...
    if (jcoord == null) {
      logger.severe(
        LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_REQUEST_INITIATED_TO_REMOVE_MEMBER_0_BUT_THERE_IS_NO_GROUP_COORDINATOR,
        mbr);
      return false;
    }
    else {
...
//        Assert.assertTrue(false, "messaging contract failure");
      ...
      if (logger.fineEnabled()) {
        logger.fine("Membership: directChannelSend caught exception " + e);
      }
      ...
      throws NotSerializableException
  {
    ...
    if (destinations == null) {
      if (logger.finerEnabled())
        logger.finer("Membership: Message send: returning early because null set passed in: {"
            + msg + "}");
...
//      }
    ...
    boolean sendViaJGroups = isForceUDPCommunications() || msg.sendViaJGroups();
    ...
    if (useMcast) {
      if (DistributionManager.VERBOSE || logger.fineEnabled())
        logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_SENDING__0__VIA_MULTICAST, msg);
...
//      else
      ...
      { // fix for bug 34010
        Thread t = new Thread() {
          ...
          public void run() {
            ...
            if (logger.fineEnabled())
              logger.fine("Membership: closing connections for departed member " + member);
  ...
  protected boolean isShunnedNoSync(DistributedMember m) {
      ...
      if (VERBOSE_VIEWS || logger.fineEnabled()) {
        logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_NO_LONGER_SHUNNING___0_, m);
      }
      endShun(m);
      return false;
  }
 ...
 /**
  ...
  protected void addShunnedMember(InternalDistributedMember m) {
    ...
    if (!isShunned(m)) {
      ...
      if (VERBOSE_VIEWS || logger.fineEnabled())
        logger.info(LocalizedStrings.JGroupMembershipManager_MEMBERSHIP_ADDED_SHUNNED_MEMBER__0_, m);
    }
    ...
    while (it.hasNext()) {
      ...
      if (logger.fineEnabled())
        logger.fine("Membership: finally removed shunned member entry <" + mm + ">");
...
//        logger.fine("DEBUG: created new dm addr " + mbr);
        // if a fully formed address, retain it in the map for future use
        if (sender.getVmKind() != 0 && createIfAbsent) {
//          logger.fine("DEBUG: adding to map " + sender + "->" + mbr);
...
//      logger.fine("DEBUG: returning " + mbr);
  ...
  public boolean waitForDeparture(DistributedMember mbr) throws TimeoutException, InterruptedException {
    ...
    do {
      ...
      if (dc != null) {
        ...
        if (wait && logger.fineEnabled()) {
          logger.fine("waiting for receivers for " + mbr + " to shut down");
        }
      }
      if (!wait) {
        latestViewLock.writeLock().lock();
    ...
    if (logger.fineEnabled()) {
      logger.fine("operations for " + mbr + " should all be in the cache at this point");
    }
    return result;
  }
  ...
  protected synchronized void beSick() {
    if (!beingSick) {
      beingSick = true;
      logger.info(LocalizedStrings.TESTING, "JGroupMembershipManager.beSick invoked for " + this.myMemberId
          + " - simulating sickness");
      fdProtocol.beSick();
  ...
  protected synchronized void playDead() {
    if (!playingDead) {
      playingDead = true;
      logger.info(LocalizedStrings.TESTING, "JGroupMembershipManager.playDead invoked for " + this.myMemberId);
      // close current connections and stop accepting new ones
  ...
  protected synchronized void beHealthy() {
    if (beingSick || playingDead) {
      beingSick = false;
      playingDead = false;
      logger.info(LocalizedStrings.TESTING, "JGroupMembershipManager.beHealthy invoked for " +
          this.myMemberId + " - recovering health now");
   ...
   * @param src
   * @param timeOffset
   * @see DistributionManager#setCacheTimeOffset(DistributedMember, long, boolean)
   */
  public void setCacheTimeOffset(Address src, long timeOffset, boolean isJoin) {
    ...
    if (this.listener != null) {
      ...
      dm.setCacheTimeOffset(coord, timeOffset, isJoin);
    }
  }
